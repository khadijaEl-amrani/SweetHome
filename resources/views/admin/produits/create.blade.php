...
@section('content')
    <div class="mb-6">
        <a href="{{ route('admin.produits.index') }}" class="text-pink-600 hover:text-pink-800">
    ...
    <form action="{{ route('admin.produits.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="designation" id="designation" value="{{ old('designation') }}" class="mt-1 focus:ring-pink-500 focus:border-pink-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
            </div>
            ...
            <div>
                ...
                <div class="mt-1 relative rounded-md shadow-sm">
                    ...
                    <input type="number" name="prix" id="prix" step="0.01" min="0" value="{{ old('prix') }}" class="focus:ring-pink-500 focus:border-pink-500 block w-full pl-7 pr-12 sm:text-sm border-gray-300 rounded-md">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="sous_famille_id" id="sous_famille_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-pink-500 focus:border-pink-500 sm:text-sm">
                    ...
                    @foreach($sousFamilles as $sousFamille)
                        <option value="{{ $sousFamille->id }}" {{ old('sous_famille_id') == $sousFamille->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4" class="mt-1 focus:ring-pink-500 focus:border-pink-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('description') }}</textarea>
        </div>
