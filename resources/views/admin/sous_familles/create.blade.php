...
@section('content')
    ...
    <form action="{{ route('admin.sous-familles.store') }}" method="POST" class="space-y-6">
        @csrf
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label for="sous_famille" class="block text-sm font-medium text-gray-700">Nom de la sous-famille</label>
                <input type="text" name="sous_famille" id="sous_famille" value="{{ old('sous_famille') }}" class="mt-1 focus:ring-pink-500 focus:border-pink-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
            </div>
